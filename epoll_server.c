#include <stdio.h>  
#include <string.h>  
#include <stdlib.h>  
#include <errno.h>  
#include <unistd.h>     
#include <arpa/inet.h>   
#include <sys/types.h>  
#include <sys/socket.h>  
#include <netinet/in.h>  
#include <sys/time.h> 
#include <poll.h>
#include <sys/epoll.h>

// Function to calculate the factorial
unsigned long long factorial(int n){
    if(n==0 || n==1){
        return 1;
    }
    if(n>=20){
        return 2432902008176640000;
    }
    unsigned long long fact[n+1];
    fact[0]=1;
    fact[1]=1;
    for(int i=2;i<=n;i++){
        fact[i]=i*fact[i-1];
    }
    return fact[n];
}

int main(){
    int serverSocket = socket(AF_INET, SOCK_STREAM, 0);
    if (serverSocket == -1) {
        perror("Error in socket creation");
        exit(1);
    }

    struct sockaddr_in serverAddress;
    serverAddress.sin_family = AF_INET;
    serverAddress.sin_port = htons(8087); //PORT NUMBER
    serverAddress.sin_addr.s_addr = INADDR_ANY;

    if (bind(serverSocket, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) == -1) {
        perror("Error in binding");
        close(serverSocket);
        exit(1);
    }

    if (listen(serverSocket, 4000) == -1) {
        perror("Error in listening");
        close(serverSocket);
        exit(1);
    }
    printf("Server is listening...\n");

    struct epoll_event ev, events[4000];
    int epollfd = epoll_create1(0);
    if(epollfd==-1){
        perror("Error in epoll_create1");
        close(serverSocket);
        exit(1);
    }
    ev.events = EPOLLIN;
    ev.data.fd = serverSocket;
    if(epoll_ctl(epollfd, EPOLL_CTL_ADD, serverSocket, &ev)==-1){
        perror("Error in epoll_ctl");
        close(serverSocket);
        exit(1);
    }
    int clientSocket[4000];
    for(int i=0;i<4000;i++){
        clientSocket[i]=0;
    }
    while(1){
        int nfds = epoll_wait(epollfd, events, 4000, -1);
        if(nfds==-1){
            perror("Error in epoll_wait");
            close(serverSocket);
            exit(1);
        }
        for(int i=0;i<nfds;i++){
            if(events[i].data.fd==serverSocket){
                struct sockaddr_in clientAddress;
                socklen_t clientAddressLength = sizeof(clientAddress);
                printf("Waiting for a client to connect...\n");
                int clientSocket = accept(serverSocket, (struct sockaddr*)&clientAddress, &clientAddressLength);
                if(clientSocket==-1){
                    perror("Error in accepting connection");
                    close(serverSocket);
                    exit(1);
                }
                ev.events = EPOLLIN;
                ev.data.fd = clientSocket;
                if(epoll_ctl(epollfd, EPOLL_CTL_ADD, clientSocket, &ev)==-1){
                    perror("Error in epoll_ctl");
                    close(serverSocket);
                    exit(1);
                }
            }
            else{
                char buffer[1024];
                int data = recv(events[i].data.fd, buffer, sizeof(buffer), 0);
                if(data==-1){
                    perror("Error in receiving");
                    close(serverSocket);
                    exit(1);
                }
                if(data==0){
                    printf("Client disconnected\n");
                    close(events[i].data.fd);
                    continue;
                }
                int n = atoi(buffer);
                unsigned long long result = factorial(n);
                char res[1024];
                sprintf(res, "%llu", result);
                if(send(events[i].data.fd, res, sizeof(res), 0)==-1){
                    perror("Error in sending");
                    close(serverSocket);
                    exit(1);
                }
            }
        }
    }
    close(serverSocket);
    printf("Server socket closed\n");
    return 0;
}